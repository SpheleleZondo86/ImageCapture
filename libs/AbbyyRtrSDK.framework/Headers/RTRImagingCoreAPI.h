 ...
 * Load image.
 */
- (id<RTRCoreAPIImage>)loadImage:(UIImage*)image error:(NSError**)error;
...
@property (nonatomic, strong, readonly) NSArray<RTRQualityAssessmentForOCRBlock*>* qualityAssessmentBlocks;
...
typedef NS_ENUM(NSUInteger, RTRCoreAPIExportCompressionLevel) {
	/** The lowest compression rate, that still might hav any noticeable effect on recognition of small text. */
