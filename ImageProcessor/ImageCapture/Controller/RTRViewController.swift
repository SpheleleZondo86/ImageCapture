...
class RTRViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad()
    {
        ...
        captureButton.setTitle("Stop", for: UIControlState.selected)
        captureButton.setTitle("Start", for: UIControlState.normal)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        cell.accessoryType = selectedDocument == document ? UITableViewCellAccessoryType.checkmark : UITableViewCellAccessoryType.none
        ...
        if isReadyForCapturing {
            ...
            DispatchQueue.global(qos: DispatchQoS.QoSClass.default).async {
                ...
                if let boundary = documentBoundary {
                    if boundary.count != 0 {
                        if let coreAPI = rtrEngine.createCoreAPI() {
                            do {
                                let rtrImage = try coreAPI.load(capturedImage)
                                if let cropOperation = coreAPI.createCropOperation() {
            ...
            let blocks = status.qualityAssessmentForOCRBlocks
            if blocks != nil {
